...
Routes allow you to map urls to controller classes and methods. If you are
already familiar with Ruby on Rails you will notice almost similar behavior.

=head2 Terms

There are a couple of terms to know. B<Url> refers to a request path. B<Map>
is the rule that matches a given path. B<Route> is the path to a class and
method to be called with additional parameters. It is possible to convert a
B<Map> to a B<Route> and to convert a B<Route> to a B<Url>.
...
important, because they decide what is going to be called if a url matches
the given map.
...
Maps can be more specific and automatically provide parameters through
placeholders.
...
Don't repeat yourself. For a given root map you can have multiple nested
maps. This can be used to factor out repetitive code.
...
Sometimes it is neccessary to call multiple method after another (e.g.
authorization, data processing). That's what gates are for.

    # /blog
    $r->gate->to(controller => 'foo', action =>'auth')
...
Waypoint is something in between gates and nested routes. It can match even
