...
One-time grants for contiguous full-time development, over the period of a few
days, can also have a huge impact. They allow us to tackle complex issues that
are otherwise left to linger until somebody can't afford to not fix them. To
arrange such a grant for a core developer please email C<sri@cpan.org>.
