...
(C<kraih@mojolicious.org>), and give us a few days to develop and release a
proper fix.
...
voting stage, someone with vote calling permissions needs to call for a vote.
You are responsible for securing this vote call as well as the required votes.
The exact L</"Voting Rules"> can be found below.
...
Additionally for GitHub pull requests, everyone who has had one of their pull
requests accepted previously (C<Contributor>) can call for a vote with a GitHub
comment of the form C<Calling for a vote @mojolicious/core>.

A feature can be added or modified when at least 3 voting members of the core
team have cast a vote in favour, or the BDFL overruled the vote.
...
vote, and require a new vote to be called. Everyone is encouraged to vote, even
for already merged pull requests, because it is very beneficial for us to know
how popular features really are.
...
Sebastian has veto rights on all decisions and will resolve issues that could
not be decided with a vote.
