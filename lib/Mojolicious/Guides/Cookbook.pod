...
downtime software upgrades.

    % ./script/myapp daemon_prefork --listen file:///tmp/myapp.sock\
      --pid /tmp/myapp1.pid --lock /tmp/myapp.lock --daemonize
    Server available at file:///tmp/myapp.sock.

All you have to do is to update your application code, start a second web
server instance and after that send a C<USR1> signal to the old instance.
