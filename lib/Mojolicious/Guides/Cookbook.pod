...
can keep many parallel connections active at the same time.
  ...
  # FIFO queue
  my @queue = (my $entry = Mojo::URL->new('http://localhost/manual'));
  ...
  my $crawl; $crawl = sub {
    ...
    if (my $url = shift @queue) {
      ...
      $ua->get($url => sub {
        ...
        $tx->res->dom('a[href]')->each(sub {
          ...
          # Build absolute URL and remove fragment
          my $url = Mojo::URL->new($e->{href})->to_abs($tx->req->url);
          $url->fragment(undef);

          # Constraint our crawler
          if (not exists $links{$url} and lc $entry->host eq lc $url->host) {
            say " -> $url";
        ...
        # Spawn  up to 4 parallel crawlers
        $crawl->() for $active-- .. 4;
      });
    }
    # No more URLs in the queue
      ...
      Mojo::IOLoop->stop unless $active;
    }
  };
