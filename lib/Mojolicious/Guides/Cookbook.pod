...
On UNIX platforms you can also add pre-forking to the built-in web server and switch to a multi-process architecture
...
For bigger applications L<Mojolicious> contains the UNIX optimized pre-forking web server
L<Hypnotoad|Mojo::Server::Hypnotoad>, which can take advantage of multiple CPU cores and copy-on-write memory
...
It is based on the L<Mojo::Server::Prefork> web server, which adds pre-forking to L<Mojo::Server::Daemon>, but
optimized specifically for production environments out of the box. To start applications with it you can use the
L<hypnotoad> script, which listens on port C<8080>, automatically daemonizes the server process and defaults to
...
One of the most popular setups these days is L<Hypnotoad|Mojo::Server::Hypnotoad> behind an L<Nginx|http://nginx.org>
...
Another good reverse proxy is L<Apache|http://httpd.apache.org> with C<mod_proxy>, the configuration looks quite
...
can be a very powerful tool especially for testing web application.
  ...
  # Extract headings
  $res->dom('h1, h2, h3')->each(sub ($dom, $i) { say 'Heading: ', $dom->all_text; });
...
them using their full class name.
  ...
  get '/' => sub ($c) { $c->my_helpers->render_with_header(text => 'I ♥ Mojolicious!'); };
...
If you have any questions the documentation might not yet answer, don't hesitate to ask on the L<mailing
list|http://groups.google.com/group/mojolicious> or the official IRC channel C<#mojo> on C<irc.freenode.net> (L<chat
now!|https://webchat.freenode.net/#mojo>).
