...
Recipes that shouldn't be used... ever, we do it so you don't have to!

=head2 IIS6.0/FCGI

This recipe is not here because we suggest using IIS, it is a horribly broken
web server, avoid it if you can.
There is not much we could do to make this a pleasant experience for you, but
maybe we can at least ease some of the pain.

First you should make sure to get a recent version of C<Strawberry Perl> and
L<Mojo::Server::FCGI> installed, it is as good as a Windows version of Perl
can be.

    % cpan Mojo::Server::FCGI
    ...
...
Click on the C<Configuration> button and then C<Insert> next to the
C<Wildcard Application Maps>.
...
or C<c:\windows\syswow64\inetsrv\fcgiext.ini> on 64-bit systems.
    ...
    *=MyApp
    ...
    Arguments="c:\myapp\script\myapp fcgi"
