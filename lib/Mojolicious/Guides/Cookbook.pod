...
L<PSGI> is an interface between Perl web applications and web servers, and
...
L<Plack> provides many server and protocol bindings for you to choose from
such as C<FCGI>, C<SCGI> and C<mod_perl>.

    % plackup ./script/myapp -s FCGI -l /tmp/myapp.sock

Some server bindings might ask for a C<.psgi> file, if thats the case you can
