...
make sense for a standalone parser.
  ...
  my $tx     = $client->get('mojolicio.us/perldoc');
...
implementation L<Mojo::JSON> built right in.
  ...
  for $tweet (@{$client->get($search)->res->json->{results}}) {
...
You can just add username and password to the URL.
  ...
  my $client = Mojo::Client->new;
  print $client->get('https://sri:secret@mojolicio.us/hideout')->res->body;

=head2 Decorating Followup Requests

L<Mojo::Client> can automatically follow redirects, the C<on_start> callback
...
initialized and before a connection gets associated with them.
  ...
  my $client = Mojo::Client->new(max_redirects => 10);
...
When downloading large files with L<Mojo::Client> you don't have to worry
...
above C<250KB> into a temporary file.
  ...
  my $client = Mojo::Client->new(max_redirects => 5);
  my $tx     = $client->get('latest.mojolicio.us');
...
Uploading a large file is even easier.
  ...
  my $client = Mojo::Client->new;
  $client->post_form('mojolicio.us/upload',
...
streamed directly from the file.
  ...
  my $tx     = $client->build_tx(PUT => 'mojolicio.us/upload');
  $tx->req->content->asset($asset);
  $client->start($tx);

=head2 Parallel Requests

You can perform multiple requests asynchronously just by adding callbacks.

  # Scrape title from main site
  my $client = Mojo::Client->new;
  $client->get('http://mojolicio.us' => sub {
    print 'Title: ', shift->res->dom->at('head > title')->text, "\n";
  });

  # Scrape headers from documentation site
  $client->get('http://mojolicio.us/perldoc' => sub {
    shift->res->dom('h1, h2, h3')->each(sub {
      print 'Header: ', shift->all_text, "\n";
    });
  });
...
The C<start> method will block until all requests are finished.

=head2 Non-Blocking

L<Mojo::Client> has been designed from the ground up to be non-blocking, the
whole blocking API is just a simple convenience wrapper.
...
time.
  ...
  # Client following up to 5 redirects
  my $client = Mojo::Client->new(max_redirects => 5);
  ...
  $crawl = sub {
    ...
    return $client->async->ioloop->timer(2 => sub { $crawl->($id) })
      unless my $url = shift @urls;

    # Fetch
    $client->async->get($url => sub {
      my $tx = pop;
  ...
  # Start a bunch of parallel crawlers sharing the same client
  $crawl->($_) for 1 .. 3;
...
You can just pick the parts that actually matter with the CSS3 selectors of
L<Mojo::DOM>.
...
The request can be customized as well.
  ...
  % mojo get --method post --header 'X-Foo: Bar' --content 'Hi!' kraih.com h1
  ...
