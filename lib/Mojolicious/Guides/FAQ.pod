...
certificate validation in L<Mojo::UserAgent> failed. This usually happens for
two reasons. The most common one is that the peer certificate is simply invalid.
If that's the case you can use the attribute L<Mojo::UserAgent/"insecure"> or
C<MOJO_INSECURE> environment variable to disable certificate validation. And if
that's not the case you might be missing the L<Mozilla::CA> module, which is
required by L<IO::Socket::SSL> to be able to validate certificates.
