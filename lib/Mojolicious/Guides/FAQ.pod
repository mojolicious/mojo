...
community and receive bug fixes, which are currently 5.24.x and 5.22.x.
...
Windows is fully supported by L<Mojolicious>, some more advanced features, such
as L<subprocesses|Mojo::IOLoop/"subprocess"> and the
L<Hypnotoad|Mojo::Server::Hypnotoad> web server, will however require the use of
the L<Linux Subsystem for Windows|https://msdn.microsoft.com/commandline/wsl/>.
