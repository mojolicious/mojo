...
REST is a software architectual style for distributed hypermedia systems such
as the web.
...
The C<defaults> method from the L<Mojolicious> instance can be used to make
our C<model> available to all actions through the C<stash>.
...
Sessions in L<Mojolicious> pretty much C<just work> out of the box and there
