...
could do.
  ...
  helper debug => sub ($c, $str) { $c->app->log->debug($str); };

  get '/' => sub ($c) { $c->debug('Hello from an action!'); } => 'index';
...
L<Mojolicious::Plugin::DefaultHelpers/"reply-E<gt>file">.
  ...
  get '/leak' => sub ($c) { $c->reply->file('/etc/passwd'); };
...
Some helpers might be useful enough for you to share them between multiple applications, plugins make that very simple.
  ...
  sub register ($self, $app, $conf) { $app->helper(debug => sub ($c, $str) { $c->app->log->debug($str); }); }
