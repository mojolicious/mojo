...
If you use 'content' helper with template block as last argument you can
pass arguments to it.
  ...
  <!DOCTYPE html>
  <html>
    <body><%= content 'block', qw/ wold! Hello / %></body>
  </html>

In this context 'content' helper behaves as L<"/Reusable template blocks"> and
have no any advantages over it, but you get more power when do
...
You can even pass arguments to content helper you extends. Here we pass $row
value from extended template to its child.
...
This example will generate next output.
