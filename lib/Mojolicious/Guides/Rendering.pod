...
=head2 Rendering base64 encoded content

It's possible to store base64 encoded images inside the templates. In order
to serve the files from the C<DATA> section a static route is needed.
...
actual files in the C<templates> directory. This includes base64 encoded files,
too.
