...
Other similar helpers allow you for example to generate HTML tags, but more
about those later.
...
Blocks start with a special opening bracket at the end of the first tag and
end with a special mini tag containing only the closing bracket, whitespaces
between bracket and tag ending are not allowed.
...
Inheritance basically means that layouts can have layouts and using the C<content>
helper to make whole template sections overloadable.
...
This could go on and on to allow a very high level of template reuse.

=head2 Helpers

Helpers are little functions that are stored in the renderer, not all are
strictly template specific, thats why you can also use the C<helper>
controller method to access them.
...
C<dumper> and tag helpers, which are template specific and used mostly to
generate C<HTML> tags.
...
script, so don't forget to use the L<utf8> pragma if neccessary.
...
to do is add a new C<handler>.
    ...
    app->renderer->add_handler(
        mine => sub {
            ...
            my $content = $r->get_inline_template($c, $t);
