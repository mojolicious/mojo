...
The helper L<Mojolicious::Plugin::DefaultHelpers/"flash"> and method
...
will be ignored by default, so you have to decide which should be required or
optional before you can perform checks on their values. Every check is performed
right away, so you can use the results immediately to build more advanced
validation logic with methods like
L<Mojolicious::Validator::Validation/"is_valid">.
  ...
  get '/' => sub {
    ...
    $c->render('thanks') unless $v->has_error;
  } => 'index';
