...
end with a special mini tag containing only the closing bracket.
Whitespace characters between bracket and tag are not allowed to
differentiate between normal Perl code and template blocks.
...
A naive translation to normal Perl code could look like this.
