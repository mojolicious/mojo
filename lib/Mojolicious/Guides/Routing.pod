...
You can also add your own route keywords to make route generation more
expressive.
    ...
    $r->add_keyword(resource => sub {
        ...
        # Handle GET requests
        $resource->get->to('#show');

        return $resource;
    });

    # POST /user -> {controller => 'foo', action => 'create'}
    # GET  /user -> {controller => 'foo', action => 'show'}
    $r->resource('user');
