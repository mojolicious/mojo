...
normal C<GET> requests with some additional information.
  ...
  # Action
  sub echo ($self) { $self->on(message => sub ($self, $msg) { $self->send("echo: $msg"); }); }
...
route with the method L<Mojolicious::Routes::Route/"over">, and allow even more powerful route constructs.
...
Post-processing the response to add or remove headers is a very common use.
  ...
  $app->hook(after_static => sub ($c) { $c->res->headers->cache_control('max-age=3600, must-revalidate'); });

  # Remove a default header
  $app->hook(after_dispatch => sub ($c) { $c->res->headers->remove('Server'); });
...
You can even extend much of the core functionality.
  ...
  $app->hook(around_action => sub ($next, $c, $action, $last) { return $c->$action($c->current_route); });
...
With the method L<Mojolicious::Routes::Route/"detour"> which is very similar to L<Mojolicious::Routes::Route/"to">, you
can allow the route to partially match and use only the remaining path in the embedded application, the base path will
