...
But note that by default the original path will simply be reused for the
...
A minimal embeddable application is nothing more than a class with a
C<handler> method, accepting L<MojoX::Controller> objects.
...
application.
    ...
    # /foo/*
    $r->route('/foo')->detour('bar#');

The remaining path gets always stored in the C<path> stash value.
    ...
    get '/bye' => sub {
        my $self = shift;
        $self->render(text => 'Bye Mojo!');
    };

    1;

Instead of using C<detour> you could also just capture your own C<path>.

    # /foo/*
    $r->route('/foo/(*path)')->to('bar#');
