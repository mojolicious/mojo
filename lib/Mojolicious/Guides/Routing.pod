...
The foundation of every web framework is a tiny blackbox connecting incoming
requests with code generating the appropriate response.

    GET /user/show/1 -> $self->render(text => 'Sebastian!');

This backbox is usually called a dispatcher.
...
Thats why regular expressions are a commonly used to make the dispatch
...
placeholder.
One or more placeholders can be everywhere in the route.
