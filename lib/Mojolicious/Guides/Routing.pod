...
are not endpoints themselves.
  ...
  my $articles = $r->waypoint('/articles')->to('articles#list')->name('list_articles');
  $articles->route('/create')->to('#create')->name('create_article');
  my $article = $articles->route('/id', id => qr/\d+/)->to('#view')->name('view_article');
  $article->route('/edit')->to('#edit')->name('edit_article');
