...
same code.
You can also mention a format in the route pattern to only match one.

  # /foo.txt -> {controller => 'foo', action => 'text', format => 'txt'}
  $r->route('/foo.txt')->to(controller => 'foo', action => 'text');

  # /foo      -> {controller => 'foo', action => 'html'}
  # /foo.html -> {controller => 'foo', action => 'html', format => 'html'}
  $r->route('/foo')->to(controller => 'foo', action => 'html');

Just make sure the more specific ones go first.
