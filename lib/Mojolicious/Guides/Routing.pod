...
router and it's underlying concepts.
...
=head2 Route To Class (C<namespace>, C<class> and C<method>)

From time to time you might want to dispatch to a controller class following
a different naming scheme, for this case there are a few extra stash values
available.

    # /bye -> MyApp::Controller::Foo->bye
    $r->route('/bye')
      ->to(namespace => 'MyApp::Controller', class => 'Foo', method => 'bye');
...
This is where conditions come into play, they are basically router plugins.
    ...
    $r->add_condition(
        agent => sub {
            ...
            # Match "User-Agent" header and return new stash on success
...
=head2 Embedding Applications (C<app>)

You can easily embed whole applications using the C<app> and C<path> stash
values.
C<app> has to be a class or L<Mojolicious> application object and C<path>
needs to contain the new path the embedded application will use for
redispatching.
    ...
    sub startup {
        ...
        # Embedded Mojolicious::Lite app under "/foo/*"
        $r->route('/foo/(*path)')->to(app => 'MyApp::Foo');
    }
