...
C<controller> hierarchies.
    ...
    # /bye -> {controller => 'foo-bar', action => 'bye'} -> MyApp::Foo::Bar->bye
    $r->route('/bye')->to('foo-bar#bye');
...
The C<class> value is entirely optional, so you can just dispatch to a
specific class using C<namespace>.

    # /bye -> MyApp::Controller::Foo->bye
    $r->route('/bye')
      ->to(namespace => 'MyApp::Controller::Foo', method => 'bye');
