...
A test file for a simple web application might look like:
  ...
  $t->post_ok('/notifications', json => {event => 'full moon', via => 'sms', channel => '+15555555555'})
    ->status_is(201)
  ...
  $t->post_ok('/insurance', form => {name => 'Jimmy', amount => '3.000.000', currency => '€'})
    ->status_is(200);
...
when the C<enable_weather> configuration value is set:
  ...
  if ($config->{enable_weather}) {
      $r->get('/weather')->to(cb => sub { shift->render(text => "It's hot! 🔥") }
  }
...
Authorization header:
  ...
  app->helper(basic_auth => sub {
    my $c = shift;
    return {Authorization => 'Basic ' . b64_encode join(':' => @_), ''};
  });
...
L<Test::Mojo>. There are four broad categories of assertions for HTTP
requests:
...
status code testing, you can access the response internals directly:
...
object, setting the cookie (see L<Mojo::Message::Request>), then passing the
transaction object to C<request_ok>:
...
communications can be tricky. The L<Test::Mojo> WebSocket assertions serialize
messages via event loop primitives. This enables us to treat WebSocket
messages as if they were using the same request-response communication pattern
we're accustomed to with HTTP.
...
WebSocket route at C</echo> and that it's "speaking" WebSocket protocol to
us. The next C<send_ok> assertion tests the connection again (in case it
closed, for example) and attempts to send the message C<Hello Mojo!>. The next
assertion, C<message_ok>, blocks (using the L<Mojo::IOLoop> singleton in the
application) and waits for a response from the server. The response is then
compared with C<'echo: Hello Mojo!'> in the C<message_is> assertion, and
finally we close and test our connection status again with C<finish_ok>.
...
L<Mojo::UserAgent::Transactor>) to marshal hash and array references into JSON
documents, and then send them as a WebSocket message. We wait (block) for a
response from the server with C<message_ok>. Then because we're expecting a
JSON document back, we can leverage C<json_message_ok> which parses the
WebSocket response body and returns an object we can access through
L<Mojo::JSON::Pointer> syntax. Then we close (and test) our WebSocket
connection.
...
In some situations you may find your tests have a lot of boilerplate, making
tests cluttered and hard to follow. For example let's say we have an RPC-based
web application where the HTTP method is always C<POST> and the remote
procedure name is in the URL, and we use HTTP Basic authentication with
...
To test this kind of web application, we'd need to do something like this for
every test:

  $t->post_ok('/thermostat/heat',
              {Authorization => 'Basic ' . b64_encode 'me:secretly', ''},
              json => {temperature => '23°C'})
    ->status_is(200);
...
First, we create a new package that will implement the role we want:
  ...
  sub post_with_auth_ok {
      ...
      $t->SUPER::post_ok($url, $headers, @_);
  }

  1;

This role accepts an extra credentials parameter, then uses that to create an
C<Authorization> header, then we pass everything along to C<post_ok> to do the
heavy lifting.

In our test file, we apply the role using C<with_roles> (available in
Mojolicious 7:41 and later), which creates a new class whose objects have the
C<post_with_auth_ok> capability:
...
=head3 Extending with multiple roles

We can compose multiple roles into L<Test::Mojo> objects, too. Let's say we
want to test the C<Location> header after a redirect, in addition to the
C<Authorization> header role we've just added. Let's create a new class that
...
that match other C<*_is> methods in L<Test::Mojo>, so we accept the test
object, the value to compare, and an optional description.

In this method we assign a default description value (C<$desc>), set the
L<Test::Builder> C<Level> global variable one level higher (which tells
L<Test::Builder> how far up the call stack to look when something fails), then
we use L<Test::More>'s C<is> method to compare the location header with the
expected header value. We wrap that in the C<success> attribute, which records
the boolean test result and propagates the L<Test::Mojo> object for method
chaining.

With this new package, we're ready to compose a new test object that
implements both roles:

  my $t = Test::Mojo->with_roles('Test::Mojo::Role::Authorization',
                                 'Test::Mojo::Role::Location')->new('MyApp');

  $t->post_with_auth_ok('/redirect/mojo', 'me:secretly',
                        json => {message => 'Mojo, here I come!'})
    ->status_is(302)
