...
sub _render {
  ...
  # Cached
  if ($mt->compiled) {
    $c->log->debug("Rendering cached @{[$mt->name]}");
  ...
  # Not cached
    ...
    # Inline
    if (defined $inline) {
      $c->log->debug(qq{Rendering inline template "$name"});
