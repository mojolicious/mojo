...
subtest "Non-blocking kept alive proxy request" => sub {
...
subtest "Blocking proxy requests" => sub {
  $ua->proxy->https("http://sri:secr3t\@127.0.0.1:$proxy");
...
subtest "Blocking proxy request again" => sub {
  my $tx = $ua->get("https://127.0.0.1:$port/proxy");
  is $tx->res->code, 200,                             'right status';
  is $tx->res->body, "https://127.0.0.1:$port/proxy", 'right content';
};

subtest "Failed TLS handshake through proxy" => sub {
...
subtest "Idle connection through proxy" => sub {
...
subtest "Blocking request to bad proxy" => sub {
...
subtest "Non-blocking request to bad proxy" => sub {
  my $err = undef;
  $ua->get(
    "https://127.0.0.1:$port/proxy" => sub {
      my ($ua, $tx) = @_;
