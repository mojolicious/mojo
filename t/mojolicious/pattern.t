...
$pattern = Mojolicious::Routes::Pattern->new->parse('/test/<controller>/:action');
$pattern->defaults({action => 'index'});
is_deeply $pattern->match('/test/foo/bar', 1), {controller => 'foo', action => 'bar'},   'right structure';
is_deeply $pattern->match('/test/foo'),        {controller => 'foo', action => 'index'}, 'right structure';
is_deeply $pattern->match('/test/foo/'),       {controller => 'foo', action => 'index'}, 'right structure';
...
is_deeply $pattern->match('/test/123/456',     1), {a => 'a', b => 'b'}, 'right structure';
is_deeply $pattern->match('/test/c/123/456',   1), {a => 'c', b => 'b'}, 'right structure';
...
# Regex in pattern
$pattern = Mojolicious::Routes::Pattern->new('/test/<controller>/:action/<id>', id => '\d+');
...
$pattern->defaults({action => 'index'});
is_deeply $pattern->match('/footest/bar'), {controller => 'foo', action => 'bar'}, 'right structure';
is $pattern->render({controller => 'zzz', action => 'lala'}), '/zzztest/lala', 'right result';
ok !$pattern->match('/test/lala'), 'no result';

# Relaxed
$pattern = Mojolicious::Routes::Pattern->new('/test/#controller/:action');
is_deeply $pattern->match('/test/foo.bar/baz'), {controller => 'foo.bar', action => 'baz'}, 'right structure';
...
# Wildcard
$pattern = Mojolicious::Routes::Pattern->new('/test/<:controller>/<*action>');
is_deeply $pattern->match('/test/foo/bar.baz/yada'), {controller => 'foo', action => 'bar.baz/yada'}, 'right structure';
is $pattern->render({controller => 'foo', action => 'bar.baz/yada'}), '/test/foo/bar.baz/yada', 'right result';
$pattern = Mojolicious::Routes::Pattern->new('/tset/:controller/*action');
is_deeply $pattern->match('/tset/foo/bar.baz/yada'), {controller => 'foo', action => 'bar.baz/yada'}, 'right structure';
is $pattern->render({controller => 'foo', action => 'bar.baz/yada'}), '/tset/foo/bar.baz/yada', 'right result';
...
is_deeply $pattern->match('/test(test)(\Qtest\E)('), {test => 'test(test)(\Qtest\E)('}, 'right structure';
...
ok $pattern->regex, 'regex has been compiled on demand';
is_deeply $pattern->match('/test.json', 1), {action => 'index'}, 'right structure';
...
is_deeply $result, {test => 'foo', action => 'index', format => 'html'}, 'right structure';
...
# Pattern reuse
$pattern = Mojolicious::Routes::Pattern->new->parse('/first')->parse('/second')->defaults({test => 'works'});
...
# Placeholder types
$pattern = Mojolicious::Routes::Pattern->new->types({num => qr/\d+/})->parse('/foo/<bar:num>/baz');
