...
$t->websocket_ok('/echo')->send_ok([0, 0, 0, 0, 2, 'a'])->send_ok([0, 0, 0, 0, 0, 'b'])->send_ok([1, 0, 0, 0, 0, 'c'])
...
$t->websocket_ok('/json')->send_ok({json => {test => 23, snowman => '☃'}})
  ...
  ->json_message_has('/2', 'with description')->json_message_hasnt('/5')->json_message_hasnt('/5', 'with description')
  ->json_message_is('/2' => 3)->json_message_is('/2' => 3, 'with description')->send_ok({json => {'☃' => [1, 2, 3]}})
...
# Two responses
$t->websocket_ok('/once')->send_ok('hello')->message_ok->message_is('ONE: hello')->message_ok->message_is('TWO: hello')
  ->send_ok('hello')->message_ok->message_is('ONE: hello')->send_ok('hello')->message_ok->message_is('ONE: hello')
  ->finish_ok;
